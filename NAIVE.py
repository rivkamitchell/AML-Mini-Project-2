...
class Naive:
    ...
    def __init__(self, X, y_range): #x_range should be a discrete set(binary for now) as well as y_range
        X=np.array(X)
        self.data = X
        self.validation=X
        (self.num_exp, self.num_features)=X.shape
        self.num_features-=1 #the result was counted in
    ...
    def k_fold_crossing(self, k):
        ...
        for i in range(k):
            ...
            model=Naive(train, self.y_range)
